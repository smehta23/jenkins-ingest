...
if [ -z "$1" ]; then
    echo "Usage: $0 <path-to-repos-file>.csv"
    exit 1
fi
...
do
    ...
    if ! git clone --depth 1 --no-checkout "git@github.com:$REPO.git"; then
...
cat "$outFile" | awk 'NR<2{print $0;next}{print $0| "sort -u -f"}' > "$outFile.tmp" && mv "$outFile.tmp" "$outFile"
cleanupTmpGitDir
